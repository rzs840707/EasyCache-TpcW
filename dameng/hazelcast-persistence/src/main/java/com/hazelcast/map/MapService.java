...
public class MapService implements ManagedService, MigrationAwareService,
    ...
    public void scheduleMapStoreWrite(String mapName, Data key, Object value, long delay) {
    	...
    	MapContainer mapContainer = getMapContainer(mapName);
    	long persistencyKeyPeroid = mapContainer.getPersistencyKeyPeroid();
    	if(persistencyKeyPeroid <= 0 || persistencyKeyPeroid > delay) {
    		mapContainer.getMapStoreWriteScheduler().schedule(delay, key, value);
    	} else {
    		mapContainer.getMapStoreWriteScheduler().schedule(delay, mapContainer.getPersistencyKeyPeroid(), key, value);
    	}
    }
