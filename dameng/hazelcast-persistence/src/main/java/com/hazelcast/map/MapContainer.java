...
import com.hazelcast.persistance.PreDefaultMapStore;
...
public class MapContainer {
	...
	private final long persistencyKeyPeroid;
	private boolean storeEnable = false;
	private PersistanceConfig persistanceConfig = null;

	public boolean isStoreEnable() {
		return storeEnable;
	}
	
	public long getPersistencyKeyPeroid() {
		return this.persistencyKeyPeroid;
	}
	// leave by zhaoxin
	// || name.startsWith(HAZELCAST)
	// || name.startsWith(AS_LIST)
	// || name.startsWith(QUEUE_LIST)
	// || name.startsWith(AS_SET))
	/**
	 * according to both storeEnable and config
	 * 
	 * @param storeEnable
	 * @param pc
	 *            config
	 */
	public void setStoreEnable(boolean storeEnable, PersistanceConfig pc) {
		this.storeEnable = storeEnable;
		if (pc != null) {
			if(pc.getObmitMap() != null && pc.getObmitMap().contains(name)) this.storeEnable = false; 
			else this.storeEnable = pc.isEnabled();
		}
	}
	...
	public MapContainer(String name, MapConfig mapConfig, MapService mapService) {
		...
		persistanceConfig = nodeEngine.getConfig().getPersistanceConfig();
		setStoreEnable(false, persistanceConfig);

		if (persistanceConfig != null) {
			store = new PreDefaultMapStore();
			persistencyKeyPeroid = persistanceConfig.getPersistencyKeyPeriod() * 1000;
			// set and load the database driver, only once
			PreDefaultMapStore.loadDriver(persistanceConfig.getDriverName());
			PreDefaultMapStore.setDataSource(persistanceConfig.getUrl());

			storeWrapper = new MapStoreWrapper(store, mapConfig.getName(), persistanceConfig.isEnabled());

			// System.out.println("Map "+ name + ": enable its map store.");
		} else {
			persistencyKeyPeroid = -1L;
			storeWrapper = null;
		}
		// for persistence ,zhaohui liu
		...
		// try {
		// MapStoreFactory factory = (MapStoreFactory)
		// mapStoreConfig.getFactoryImplementation();
		// if (factory == null) {
		...
		// }
		// storeWrapper = new MapStoreWrapper(store, mapConfig.getName(),
		...
		if (storeWrapper != null) {
			if (store instanceof MapLoaderLifecycleSupport) {
				((MapLoaderLifecycleSupport) store).init(nodeEngine
						.getHazelcastInstance(), mapConfig.getMapStoreConfig()
						.getProperties(), name);
			}
			// only master can initiate the loadAll. master will send other
			// members to loadAll.
			...
			// for persistence ,zhaohui liu
			if (persistanceConfig.getWriteDelay() > 0) {
				// Multi thread persistence 
				mapStoreWriteScheduler = EntryTaskSchedulerFactory.newScheduler(nodeEngine.getExecutionService().getScheduledExecutor(name), new MapStoreWriteProcessor(this, mapService), ScheduleType.FOR_EACH);
				mapStoreDeleteScheduler = EntryTaskSchedulerFactory.newScheduler(nodeEngine.getExecutionService().getScheduledExecutor(name), new MapStoreDeleteProcessor(this, mapService), ScheduleType.FOR_EACH);
				
				// Single thread persistence
				...
				// mapStoreDeleteScheduler = EntryTaskSchedulerFactory.newScheduler(nodeEngine.getExecutionService().getScheduledExecutor(), new MapStoreDeleteProcessor(this, mapService), ScheduleType.FOR_EACH);
			} else {
	...
	public boolean isMapReady() {
		// map ready states whether the map load operation has been finished. if
		// not retry exception is sent.
		return mapReady;
	}
	...
	public void loadMapFromStore(boolean force) {
		if (force || initialLoaded.compareAndSet(false, true)) {
			mapReady = false;
			NodeEngine nodeEngine = mapService.getNodeEngine();
			int chunkSize = nodeEngine.getGroupProperties().MAP_LOAD_CHUNK_SIZE
					.getInteger();
			...
			for (Object key : keys) {
				...
				int partitionId = nodeEngine.getPartitionService()
						.getPartitionId(dataKey);
				Address partitionOwner = nodeEngine.getPartitionService()
						.getPartitionOwner(partitionId);
				while (partitionOwner == null) {
					partitionOwner = nodeEngine.getPartitionService()
							.getPartitionOwner(partitionId);
					try {
						Thread.sleep(10);
				...
				if (partitionOwner.equals(nodeEngine.getClusterService()
						.getThisAddress())) {
			...
			for (Map<Data, Object> currentChunk : chunkList) {
				try {
					nodeEngine.getExecutionService().submit("hz:map-load",
							new MapLoadAllTask(currentChunk, counter));
				} catch (Throwable t) {
	...
	// zhaohui liu , for persistence config, for asynchronize persistence
	public long getWriteDelayMillis() {
		return (persistanceConfig == null) ? (mapConfig.getMapStoreConfig()
				.getWriteDelaySeconds() * 1000) : (persistanceConfig
				.getWriteDelay() * 1000);
	...
	private class MapLoadAllTask implements Runnable {
		...
		public void run() {
			...
			for (Data dataKey : keys.keySet()) {
				...
				int partitionId = nodeEngine.getPartitionService()
						.getPartitionId(dataKey);
				PutFromLoadOperation operation = new PutFromLoadOperation(name,
						dataKey, dataValue, -1);
				...
				OperationAccessor.setCallerAddress(operation,
