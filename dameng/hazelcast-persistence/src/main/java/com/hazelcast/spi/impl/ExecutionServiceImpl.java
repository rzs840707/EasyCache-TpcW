...
public final class ExecutionServiceImpl implements ExecutionService {
    ...
    // yanshi.wys
    private final ConcurrentHashMap<String, ScheduledExecutorService> scheduledExecutors = new ConcurrentHashMap<String, ScheduledExecutorService>();
    ...
    // yanshi.wys
    public ScheduledExecutorService getScheduledExecutor(String mapName) {
    	if(mapName == null || mapName.isEmpty()) return null;
    	ScheduledExecutorService scheduledExecutorService = scheduledExecutors.get(mapName);
    	if(scheduledExecutorService == null) {
    		synchronized (mutex) {
    			scheduledExecutorService = scheduledExecutors.get(mapName);
    			if(scheduledExecutorService == null) {
    				final Node node = nodeEngine.getNode();
    				final String scheduledThreadName = node.getThreadNamePrefix("scheduled." + mapName);
    				scheduledExecutorService = Executors.newSingleThreadScheduledExecutor(new SingleExecutorThreadFactory(node.threadGroup, node.getConfig().getClassLoader(), scheduledThreadName));
    				enableRemoveOnCancelIfAvailable();
    				scheduledExecutors.put(mapName, scheduledExecutorService);
    			}
    		}
    	}
    	return new ScheduledExecutorServiceDelegate(scheduledExecutorService);
    ...
    void shutdown() {
        ...
        for(Entry<String, ScheduledExecutorService> executorEntry : scheduledExecutors.entrySet()) {
        	executorEntry.getValue().shutdownNow();
        }
