...
public class Config implements DataSerializable {
	...
	//for Persistance Initialze, add by zhaoxin ,zhaohui liu
    private PersistanceConfig persistanceConfig = new PersistanceConfig();
    public PersistanceConfig getPersistanceConfig() {
        return persistanceConfig;
    }

    public Config setPersistanceConfig(PersistanceConfig persistanceConfig) {
        this.persistanceConfig = persistanceConfig;
        return this;
    }
    
    //for IdGenerator Initialze, add by zhaohuiliu
    private Map<String, IdGeneratorConfig> idGeneratorConfigs = new ConcurrentHashMap<String, IdGeneratorConfig>();
    
    public IdGeneratorConfig getIdGeneratorConfig(final String name) {
		IdGeneratorConfig config;
		if( (config = lookupByPattern(idGeneratorConfigs, name)) != null )
			return config;
		
		IdGeneratorConfig defConfig = idGeneratorConfigs.get("default");

        if (defConfig == null) {
            defConfig = new IdGeneratorConfig();
            defConfig.setName("default");
            addIdGeneratorConfig(defConfig);
        }
        config = new IdGeneratorConfig(defConfig);
        config.setName(name);
        addIdGeneratorConfig(config);
		
		return config;
	}
    
    public IdGeneratorConfig findMatchingIdGeneratorConfig(String name) {
    	IdGeneratorConfig config;
        if ((config = lookupByPattern(idGeneratorConfigs, name)) != null) return config;
        return getIdGeneratorConfig("default");
    }

    public Config addIdGeneratorConfig(IdGeneratorConfig persistenceConfig) {
    	idGeneratorConfigs.put("i:" + persistenceConfig.getName(), persistenceConfig);
        return this;
    }

    public Map<String, IdGeneratorConfig> getIdGeneratorConfigs() {
        return Collections.unmodifiableMap(idGeneratorConfigs);
    }

    public Config setIdGeneratorConfigs(Map<String, IdGeneratorConfig> persistenceConfigs) {
        this.idGeneratorConfigs = persistenceConfigs;
        for (final Entry<String, IdGeneratorConfig> entry : this.idGeneratorConfigs.entrySet()) {
            entry.getValue().setName(entry.getKey());
        }
        return this;
    }
    
    private void checkIdGeneratorCompatible(final Config config) {
