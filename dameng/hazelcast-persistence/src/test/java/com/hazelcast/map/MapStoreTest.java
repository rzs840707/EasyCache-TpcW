...
public class MapStoreTest extends HazelcastTestSupport {
    ...
    class SimpleMapLoader implements MapStore {
        ...
        @Override
        public void store(Object key, Object value,String table) {
        }

        @Override
        public void storeAll(Map map,String table) {
        }
...
//        assertEquals(employee, values.iterator().next());
    ...
    public static class TestEventBasedMapStore<K, V> implements MapLoaderLifecycleSupport, MapStore<K, V> {
        ...
        public void store(K key, V value,String table) {
            store.put(key, value);
        ...
        public void delete(K key,String table) {
            store.remove(key);
            callCount.incrementAndGet();
    ...
    public static class MapStoreAdaptor<K, V> implements MapStore<K, V> {
        ...
        public void delete(final K key,String table) {
        }

        public void store(final K key, final V value,String table) {
        }

        public void storeAll(final Map<K, V> map,String table) {
            for (Map.Entry<K, V> entry : map.entrySet()) {
                store(entry.getKey(), entry.getValue(),null);
            }
        }

        public void deleteAll(final Collection<K> keys,String table) {
            for (K key : keys) {
                delete(key,null);
            }
        }
    ...
    public static class SimpleMapStore<K, V> extends MapStoreAdaptor<K, V> {
        ...
        @Override
        public void storeAll(final Map<K, V> kvMap,String table) {
            store.putAll(kvMap);
        }
    }
